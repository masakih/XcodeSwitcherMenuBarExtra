...
class Helper: NSObject, NSXPCListenerDelegate, HelperProtocol {
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        ...
        connection.remoteObjectInterface = NSXPCInterface(with: HelperClient.self)
    ...
    func switchDeveloperDirectory(url: URL, completion: @escaping (NSNumber) -> Void) {
        ...
        switchDeveloperDirectory(withPath: url.path, completion: completion)
    }
    
    func switchDeveloperDirectory(url: URL, authData: NSData?, completion: @escaping (NSNumber) -> Void) {
        
        switchDeveloperDirectory(withPath: url.path, authData: authData, completion: completion)
    }
    
    func switchDeveloperDirectory(withPath path: String, completion: @escaping (NSNumber) -> Void) {
        
        runTask(command: "/usr/bin/xcode-select", arguments: ["-s", path], completion: completion)
    }
    
    func switchDeveloperDirectory(withPath path: String, authData: NSData?, completion: @escaping (NSNumber) -> Void) {
        ...
        guard self.verifyAuthorization(authData, forCommand: #selector(HelperProtocol.switchDeveloperDirectory(withPath:authData:completion:))) else {
            completion(kAuthorizationFailedExitCode)
            return
        }
        
        switchDeveloperDirectory(withPath: path, completion: completion)
    ...
    private func verifyAuthorization(_ authData: NSData?, forCommand command: Selector) -> Bool {
        do {
            ...
            if let remoteObject = self.connection()?.remoteObjectProxy as? HelperClient {
